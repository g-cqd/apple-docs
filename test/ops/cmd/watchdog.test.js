...
describe('runWatchdog', () => {
  ...
  test('healthy probe after failures resets the counter (no kickstart)', async () => {
    ...
    const outcomes = [
      { ok: false, status: 500 },
      { ok: false, status: 500 }, // web/mcp it1
      { ok: false, status: 500 },
      { ok: false, status: 500 }, // web/mcp it2
      { ok: true, status: 200 },
      { ok: true, status: 200 }, // web/mcp it3 (recovered)
    ]
  ...
  test('cooldown stamp is written BEFORE the kickstart call', async () => {
    ...
    const kickstart = async () => {
      throw new Error('kickstart blew up')
    }
    const fs = inMemoryFs()
    const { ctx } = baseCtx({
      probe: probe.fn,
  ...
  test('cooldown expires after WATCHDOG_COOLDOWN seconds (next kickstart fires)', async () => {
    ...
    const sleep = async () => {
      t += 301 * 1000
    } // advance past 300s cooldown each tick
    const fs = inMemoryFs()
    const { ctx } = baseCtx({
      probe: probe.fn,
      kickstart: kick.fn,
