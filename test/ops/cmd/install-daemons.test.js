...
describe('runInstallDaemons', () => {
  ...
  test('walks the full install flow when root', async () => {
    ...
    expect(
      runner.calls.some(
        (c) => c.args[0] === '/usr/sbin/visudo' && c.args.includes('-cf') && c.args[c.args.length - 1] === '/fake/ops/launchd/sudoers.apple-docs-launchctl',
      ),
    ).toBe(true)

    // sudoers was installed to /etc/sudoers.d/mt_test-launchctl with mode 440.
    const installSudoers = runner.calls.find((c) => c.args[0] === '/usr/bin/install' && c.args.includes('440'))
    ...
    const plistInstalls = runner.calls.filter((c) => c.args[0] === '/usr/bin/install' && c.args.includes('644'))
