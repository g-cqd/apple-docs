...
import { _coalesceInflightCount, _resetCoalesceForTests, coalesceByKey } from '../../../src/pipeline/coalesce.js'
...
describe('coalesceByKey', () => {
  ...
  test('different keys do not coalesce', async () => {
    ...
    const [a, b] = await Promise.all([coalesceByKey('a', fn), coalesceByKey('b', fn)])
    expect(calls).toBe(2)
  ...
  test('all concurrent callers receive the same rejection', async () => {
    ...
    const [a, b] = await Promise.allSettled([coalesceByKey('k', fn), coalesceByKey('k', fn)])
