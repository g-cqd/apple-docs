...
describe('reader-pool (integration, real workers)', () => {
  ...
  test(
    'routes a getPage call through a worker and returns identical data',
    async () => {
  ...
  test(
    'parallel searchPages across multiple workers returns merged rows',
    async () => {
  ...
  test(
    'whitelist rejects non-read ops',
    async () => {
  ...
  test(
    'fuzzyMatchTitles runs inside a worker and returns Levenshtein matches',
    async () => {
      const pool = createReaderPool({ dbPath, size: 1 })
      await pool.start()
      try {
  ...
  test(
    'recycle reopens workers against the same DB file',
    async () => {
      const pool = createReaderPool({ dbPath, size: 2 })
      await pool.start()
      try {
