...
export function up(db) {
  ...
  try {
    db.run("ALTER TABLE roots ADD COLUMN source_type TEXT NOT NULL DEFAULT 'apple-docc'")
  } catch {
    /* column exists */
  }
  ...
  try {
    db.run("ALTER TABLE pages ADD COLUMN source_type TEXT NOT NULL DEFAULT 'apple-docc'")
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN language TEXT')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN is_release_notes INTEGER DEFAULT 0')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN url_depth INTEGER DEFAULT 0')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN doc_kind TEXT')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN source_metadata TEXT')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN min_ios TEXT')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN min_macos TEXT')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN min_watchos TEXT')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN min_tvos TEXT')
  } catch {
    /* */
  }
  try {
    db.run('ALTER TABLE pages ADD COLUMN min_visionos TEXT')
  } catch {
    /* */
  }
  ...
  db.run(
    "UPDATE pages SET source_type = (SELECT r.source_type FROM roots r WHERE r.id = pages.root_id) WHERE EXISTS (SELECT 1 FROM roots r WHERE r.id = pages.root_id AND r.source_type != 'apple-docc')",
  )
