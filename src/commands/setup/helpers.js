...
export async function extractTarZst(archivePath, dataDir) {
  ...
  // (macOS ships none).
  //
  // Disk preflight: extraction transiently needs the temp tar PLUS the
  ...
  try {
    ...
    if (exitCode !== 0) {
      throw new ValidationError(
        `Extraction failed (tar exit ${exitCode}): ${(await stderrText).trim().slice(0, 4096)}\n` +
        `The tree under ${dataDir} may be partial — re-run setup with --force once the cause is fixed.`,
      )
    }
  } finally {
