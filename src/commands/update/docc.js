...
export async function updateDoccSource(adapter, discovery, requestedRoots, concurrency, parallel, semaphore, ctx) {
  ...
  if (pages.length > 0) {
    logger.info(
      `Check complete for ${adapter.constructor.displayName}: ${modified.length} modified, ${deleted.length} deleted, ${counts.unchangedCount} unchanged, ${counts.errCount} errors`,
    )
  }
