...
export async function syncAppleResources(opts, ctx) {
  const includeFonts = opts.fonts !== false
  const includeSymbols = opts.symbols !== false
  const includePrivateSymbols = opts.privateSymbols !== false
  const downloadFonts = opts.downloadFonts === true

  const result = {
    fonts: { families: 0, files: 0, downloaded: 0, extracted: 0 },
    symbols: { public: 0, private: 0 },
  }

  if (includeFonts) {
    const fontResult = await syncAppleFonts({ downloadFonts }, ctx)
    result.fonts = fontResult
  }
  if (includeSymbols) {
    const scopes = includePrivateSymbols ? ['public', 'private'] : ['public']
    for (const scope of scopes) {
      result.symbols[scope] = await syncSfSymbols({ scope }, ctx)
    }
  }
  return result
}
...
async function readPlist(path) {
  ...
  if (code !== 0) throw new Error(`plutil failed for ${path}: ${stderr.trim()}`)
