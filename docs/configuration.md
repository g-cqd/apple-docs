...
| --- | --- | --- | --- |
| `compact` | ~4 GB (3.4) | Compresses sections, makes the body index contentless, drops the embedded raw payloads, VACUUMs | Rendered on demand |
| `balanced` *(default)* | ~6 GB (5.7) | Ships the snapshot as-is | Markdown cached on first read (7-day TTL) |
| `prebuilt` | ~9 GB (8.8) | Materializes Markdown + HTML | Served from disk (fastest) |

A full setup from a snapshot is **about 5 minutes** end to end, depending on
download speed and system load (the ~1.9 GB download dominates; post-download
work was ~40 s balanced, ~1–2 min compact, ~2 min prebuilt).

<sup>¹ Parenthesised values are measured with `du` as of `snapshot-20260608` (1.9 GB download, 340,515 documents). All three carry the same ~1.7 GB of extracted fonts, SF Symbol renders, and the ~23 MB embedding model. The variable part is the DB and rendered files:</sup>

| | DB | + Markdown | + HTML | total (`du`) |
| --- | --- | --- | --- | --- |
| `compact` | 1.8 GiB | — | — | **3.4 GB** |
| `balanced` | 4.1 GiB | — | — | **5.7 GB** |
| `prebuilt` | 4.1 GiB | 1.5 GB | 1.5 GB | **8.8 GB** |

<sup>`compact` nearly halves the DB (4.1 → 1.8 GiB) by dropping the embedded raw payloads, making the body index contentless, and zstd-compressing sections. `prebuilt`'s Markdown + HTML are only ~1.15 GB of actual content (519 MB + 631 MB across ~681k files); the rest of the 3 GB is 4 KB filesystem-block rounding on hundreds of thousands of tiny files. `apple-docs storage stats` totals logical content bytes (≈3.1 GB compact, ≈6.7 GB prebuilt), so it reads below the `du` "space consumed on the volume" figure. Install time after the ~1.9 GB download was ~40 s (balanced), ~1–2 min (compact: body reindex + VACUUM), and ~2 min (prebuilt: rendering all 340k docs).</sup>
