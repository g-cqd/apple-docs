...
This runbook assumes the standard `ops/` topology (see
[`docs/self-hosting.md`](../self-hosting.md)):

```
cloudflared tunnel ──▶ Caddy (loopback web + mcp upstreams)
                              ├──▶ Bun web server
                              └──▶ Bun mcp server
```
...
actual SSH target; `<repo>` with the repo checkout path declared in
your `ops/.env`.

## Pre-flight

1. Confirm the latest `snapshot.yml` workflow run succeeded — the
   determinism gate (re-build + sha256 diff) must be green. Failed
   determinism = do NOT deploy; investigate first.
...
previous corpus stays in place and the running servers don't get
interrupted.
...
This issues a purge of `/api/*` and the static `/data/*` artefacts, then
re-fetches the most-used endpoints to populate the edge cache from a
known-warm state.
...
`ops/bin/_exec.sh`-driven prune is run).

## Related

- `snapshot.yml` workflow — produces the artefacts.
- `release-binaries.yml` workflow — attaches standalone CLI binaries.
- [`docs/installing.md`](../installing.md) — install paths (dev / standalone / production).
- [`docs/self-hosting.md`](../self-hosting.md) — full deployment reference (launchd / Caddy / cloudflared).
- [`SECURITY.md`](../../SECURITY.md) — snapshot validation + hardened defaults.
