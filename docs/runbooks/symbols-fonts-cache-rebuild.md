# the reference host — symbols and fonts cache rebuild
...
- SSH into the reference host and `cd /opt/apple-docs` (or wherever the deploy
  checkout lives).
...
on the reference host — the renderer is single-threaded by design.
