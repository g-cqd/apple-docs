// The v1 "initial schema" migration — the VERBATIM DDL block of
// `src/storage/migrations/v1-initial-schema.js` (the union of every migration up
// to v13, replayed via one multi-statement `exec` exactly as the JS `db.exec`).
// Later steps drop parts of it again (v15 kills `refs`, v21 the legacy pages FTS
// subsystem) — that is the ladder's own history, replayed rather than folded, so
// the final catalog (incl. `sqlite_master` ordering and autoindex numbering) is
// byte-identical to a JS-migrated corpus.

import ADStorage

extension AppleDocsSchema {
    // swiftlint:disable:next function_body_length  // one verbatim JS SQL block (v1-initial-schema.js)
    static func v1(_ db: SQLiteWriteConnection) throws(SQLiteWriteError) {
        try db.exec(
            """
