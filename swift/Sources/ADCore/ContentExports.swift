...
private func payload(from bytes: [UInt8]) -> UnsafeMutableRawPointer? {
    bytes.withUnsafeBytes { raw -> UnsafeMutableRawPointer? in
...
public func adContentPageMarkdown(_ ptr: UnsafePointer<UInt8>?, _ len: Int) -> UnsafeMutableRawPointer? {
    ...
    guard let pathField = reader.nullableSpan(max: maxInputBytes),
        let rawField = reader.nullableSpan(max: maxInputBytes)
    else {
