// Regenerates the ONE committed Tier 1 fixture corpus (`Fixtures/js-corpus`) from a LIVE, scoped
// crawl of the real developer.apple.com corpus. Disabled by default — this needs network access
// plus `bun` — and is NOT a gate; the frozen, committed fixture file is what `CLIParityTests`
// compares against, exactly like ADWriteTests/SchemaParityTests.swift's `captureReferenceFixture`
// is a disabled-by-default regeneration tool for ITS committed fixture, not part of that gate
// either. Run explicitly with `AD_CAPTURE_PARITY_FIXTURE=1` when the chosen framework subset needs
// to change (e.g. one of the three frameworks below grows past "tens of pages" and stops being
// cheap/fast/deterministic).
//
// Since the storage pivot (RFC 0007 D-0007-4) BOTH engines read the same SQLite format, so the
// former second step — deriving a Swift-side ADDB fixture from this file via the (deleted)
// `ad-cli import` — is gone along with `Fixtures/swift-corpus/`: one JS-crawled file IS the whole
// fixture, and `ParityFixture` hands each engine its own fresh copy per case.
//
...
//      This produces a real, JS-crawled SQLite `apple-docs.db` — including the body-FTS pass, so
//      `capabilities.searchBody` and the body-tier search cases compare for real.
//
...
// real corpus (`ad-cli frameworks --json`, sorted by `pageCount`) for real, tiny, apple-docc-
// sourced frameworks — `adsupport` is the exact size class this RFC's own task description named
// ("accessoryaccess/adsupport-sized, tens not thousands of pages").
...
struct FixtureRegenerationTests {
    ...
    @Test(
        "regenerate the committed js-corpus fixture from a live scoped crawl",
        .enabled(
            if: ParityEnvironment.isAvailable
                && ProcessInfo.processInfo.environment["AD_CAPTURE_PARITY_FIXTURE"] != nil,
            Comment(
                rawValue: "set AD_CAPTURE_PARITY_FIXTURE=1 (with bun + network access) to "
                    + "regenerate — needs a live crawl, so it is never run as part of the gate")))
    func regenerateFixture() async throws {
        ...
        print("regenerated fixture under \(ParityEnvironment.fixturesRoot.path)")
    }
