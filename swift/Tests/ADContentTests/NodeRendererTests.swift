...
@Test func lists() throws {
    ...
    let ol: String = try render(#"[{"type":"orderedList","items":[{"content":[{"type":"paragraph","inlineContent":[{"type":"text","text":"x"}]}]}]}]"#)
...
@Test func linksBlockUsesResolvedKey() throws {
    let html: String = try render(#"[{"type":"links","items":[{"_resolvedKey":"swiftui/view","_resolvedTitle":"View"}]}]"#)
    expectEqual(html, #"<ul><li><a href="/docs/swiftui/view/">View</a></li></ul>"#)
}

@Test func referenceInlineLinksInternally() throws {
    let html: String = try render(#"[{"type":"paragraph","inlineContent":[{"type":"reference","_resolvedKey":"swiftui/text","_resolvedTitle":"Text"}]}]"#)
    expectEqual(html, #"<p><a href="/docs/swiftui/text/">Text</a></p>"#)
}

@Test func linkInlineSafetyGate() throws {
    let safe: String = try render(#"[{"type":"paragraph","inlineContent":[{"type":"link","destination":"https://x.com","title":"X"}]}]"#)
    expectEqual(safe, #"<p><a href="https://x.com">X</a></p>"#)
    let unsafe: String = try render(#"[{"type":"paragraph","inlineContent":[{"type":"link","destination":"javascript:alert(1)","title":"bad"}]}]"#)
