...
# "<dir-name> <ENV_VAR>" pairs. ADFoundation/ADJSON/ADHTML + the server engine
# (ADServe) and the HTTP stack it runs on are required for ad-server; ADBuildTools is
# consumed only under APPLEDOCS_DEV but exported anyway (the manifest ignores it
# otherwise). ADConcurrency/ADMCP/ADTestKit are NO LONGER separate siblings — they were
# folded into ADFoundation/ADServe (see AD-FAMILY-CONSOLIDATION-PLAN) and resolve
# through those packages. ADDB/ADSQL are NO LONGER dependencies at all (the storage
# pivot, RFC 0007 D-0007-4 stage 2f — the corpus is real SQLite via the dlopen'd
# libsqlite3). Package resolution is global, so a missing override here fails EVERY
# product, not just ad-server.
siblings="
