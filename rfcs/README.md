...
| [0003 — Swift render service](0003-swift-render-service.md) | P3 | Active, **reordered**: darwin-first side slice. **Phases 1–2 EXECUTED (2026-06-13)**: `render` module default-on for every darwin render — font-text + symbol-pdf + symbol-png (query, warm 163×/1497× over JIT) and the symbol prerender (`ad_render_symbol_pdf_batch`, 2.0× over the worker pool, RSS bounded). D-0003-3 settled SAFE incl. concurrent AppKit + NSBitmap PNG. Phase 3 (spawn-script kills) waits a release cycle; Linux shaper + hb-view kill deferred (phase 4, revisit-triggered) |
