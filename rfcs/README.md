...
| **P6** | Servers — web + MCP on SwiftNIO (no Vapor) | ◑ Host viable + safe (async, **no `@unchecked`** — classic EL-handler measured no faster, reverted); lexical cascade **byte-exact**, comparable at c=1 — but degrades under concurrency while Bun scales. Cause **pinned** via stage isolation: rawscan/decode/full all hit the same ~390 c16 ceiling → **NOT the serving model, NOT the cascade code** (decode/rerank/JSON cost only at c=1); it's the **SQLite scan + host topology** (same-host `ab` confound). Alloc-rewrite off the table. Fork: clean-host re-measure vs search stays Bun (cascade + serving proven good, banked for P7) |
