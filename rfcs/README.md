...
| **P6** | Servers — web + MCP on SwiftNIO (no Vapor) | ◑ Host viable + safe (async, **no `@unchecked`**); **serving-win achieved** + `/search` now **FULL byte-parity incl. enrichment** (snippet + relatedCount; zstd section codec; reuses ADContent.PlainText). The ~390 c16 ceiling was **SQLite's global `MEMSTATUS` allocator mutex** — one `sqlite3_config(MEMSTATUS,0)` (C shim): c16 **383→2263** (synthetic, ~2× Bun) / **27→51** (real 4GB, ≈ Bun 58), scales positively, TSan-clean. **Custom-scheduler plan obsoleted.** 2nd mutex (`pcache1` group, Homebrew build) caps both on real corpus — deferred. `/search` remaining: fuzzy/body/relaxation tiers, filters; then MCP protocol + web wiring |
