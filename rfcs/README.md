...
| **P6** | Servers — web + MCP on SwiftNIO (no Vapor) | ◑ Host viable + safe (async, **no `@unchecked`**); lexical cascade **byte-exact**; **serving-win achieved**. The ~390 c16 ceiling was **SQLite's global `MEMSTATUS` allocator mutex** (profiler: 8 readers ~90% blocked in `sqlite3Malloc`) — NOT the cascade, NOT QoS, NOT a scheduler. One `sqlite3_config(MEMSTATUS,0)` (6-line C shim): Swift `/search` c16 **383→2263** (synthetic, ~2× Bun) / **27→51** (real 4GB, ≈ Bun 58), scales positively, TSan-clean. **Custom-scheduler plan obsoleted.** 2nd mutex (`pcache1` group, Homebrew build) caps both on real corpus — deferred. Remaining: MCP protocol, wiring |
